...
function getCurrentSHA (author) {
  return new Promise((resolve, reject) => {
    ...
    get({
      host: 'api.github.com',
      path: `/repos/deeprado/electron-vue-webpack5/commits${isBranch ? '?sha=' + process.argv[2].split('#')[1] : ''}`,
      headers: {
        'User-Agent': author
      }
    }, res => {
...
function appendSHALink (sha, destDirName) {
  ...
  md = md.replace(
    ' using',
    `@[${sha.substring(0, 7)}](https://github.com/deeprado/electron-vue-webpack5/tree/${sha}) using`
  )
